# Plan (v02): `warc_backup_script` (cron-triggered WARC backup + tracking-sheet updates)
...
2. For each active collection, queries Archive‑It’s **WASAPI** to identify **new WARC files** using **store-time-only** discovery with a **30-day overlap window**.
...
**Locked decisions (v02)**

- Discovery clock: **`store-time` only** (no “store-time else crawl-time” logic).
- Overlap window: **30 days** (metadata re-scan; downloads remain filename-deduped).
- Retry correctness: driven by **local manifest + overlap window**; the sheet is primarily reporting/control.
...
### Checkpointing (store-time only, overlap-window safe)

The script needs a per-collection checkpoint for WASAPI discovery.

**Decision:** discovery uses **only** WASAPI `store-time` and never falls back to `crawl-time` / `crawl-start`.  
Rationale: this script is backing up what is **actually deposited and available** in storage, not tracking crawl execution.

To make store-time checkpointing **robust** (avoid “I didn’t observe it on a prior run, so now I’ll never see it again”), checkpointing must be **overlap-window safe** and filename-deduped.
...
### Decision: store-time is the only discovery clock

**The script uses `store-time` exclusively** for “what’s new?” discovery, because it best matches “now available for download / backup”.

This plan intentionally avoids any “`store-time` else `crawl-time`” fallback logic. If `store-time` is missing on a record, treat that record as a data integrity issue (log + surface) rather than silently substituting another timestamp.

### Recommended checkpoint rule (store-time + overlap window)

Checkpointing must be overlap-window safe.
...
- Do you want the “after” filter to be **inclusive or exclusive**?  
  Recommendation: treat it as **exclusive** (strictly after), and rely on filename dedupe + overlap window.

- Should the tracking sheet’s “Last WASAPI fetch” be treated as:
  - a human-facing reporting field (recommended), or
  - an operational source of truth?  
  Recommendation: the **local manifest/state** is the source of truth; the sheet can lag without breaking correctness.
...
1. Compute `after_datetime` using the **store-time + 30-day overlap window** rule.
2. Call WASAPI endpoint with:
...
- `filename` (primary identity for dedupe)
...
- Do you want to use **seed-level** logic to filter?  
  Recommendation: not initially—start collection-level and optionally populate seed-level as a reporting enhancement later.
...
6. **Error policy**
   ...
   - How long to retain “failed” entries before flagging for manual review?
