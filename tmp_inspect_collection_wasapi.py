...
PATH_UNSAFE_CHARACTERS: set[str] = set('/\\\x00\n\r\t')
IDENTIFIER_FIELD_CANDIDATES: tuple[str, ...] = (
    'identifier',
    'item_identifier',
    'archive_identifier',
    'ia_identifier',
...
FILENAME_FIELD_CANDIDATES: tuple[str, ...] = (
    'filename',
    'name',
    'original_filename',
    'original-name',
...
def get_archive_it_credentials() -> tuple[str, str]:
    ...
    result: tuple[str, str] = (username, password)
    return result
...
def build_output_paths(output_dir: Path, collection_id: int, requested_at: datetime) -> dict[str, Path]:
    ...
    Called by: main()
    """
    timestamp: str = requested_at.strftime('%Y%m%dT%H%M%SZ')
    collection_directory: Path = output_dir / f'collection_{collection_id}' / timestamp
    pages_directory: Path = collection_directory / 'pages'
...
def extract_records_from_page(page_payload: dict[str, object]) -> list[dict[str, object]]:
    ...
    for key in ('results', 'files', 'items', 'data'):
        candidate: object = page_payload.get(key)
...
def get_next_page_number(page_payload: dict[str, object], current_page_number: int) -> int | None:
    ...
    if isinstance(next_value, str) and next_value:
        if 'page=' in next_value:
            fragment: str = next_value.split('page=', 1)[1].split('&', 1)[0]
            if fragment.isdigit():
                result = int(fragment)
...
def fetch_collection_wasapi_pages(
    ...
    while True:
        ...
        try:
            ...
            page_payload: object = response.json()
            if not isinstance(page_payload, dict):
                raise ValueError('WASAPI response JSON is not an object.')
            pages.append(page_payload)
        except Exception as exc:
            request_url: str = base_url if response is None else str(response.request.url)
            if response is None:
                request_records.append(
                    RequestRecord(
                        page_number=page_number,
            ...
            partial_result: FetchResult = FetchResult(pages=pages, request_records=request_records)
            raise WasapiFetchError(f'Failed fetching page {page_number}: {exc}', partial_result) from exc

        next_page_number: int | None = get_next_page_number(page_payload, page_number)
        page_records: list[dict[str, object]] = extract_records_from_page(page_payload)
    ...
    result: FetchResult = FetchResult(pages=pages, request_records=request_records)
    return result
...
def find_first_string(record: dict[str, object], field_names: tuple[str, ...]) -> str | None:
    ...
    for field_name in field_names:
        candidate: object = record.get(field_name)
...
def build_metadata_summary(pages: list[dict[str, object]]) -> dict[str, object]:
    ...
    record_identifier_count: int = 0

    for record in records:
        filename: str | None = find_first_string(record, FILENAME_FIELD_CANDIDATES)
        if filename is not None:
            filenames.append(filename)
            anomalies: list[str] = detect_filename_anomalies(filename)
            if anomalies:
    ...
    filename_counter: Counter[str] = Counter(filenames)
    duplicate_filenames: list[str] = [filename for filename, count in filename_counter.items() if count > 1]

    flat_layout_assessment: str = 'still_unclear'
...
def save_raw_wasapi_pages(pages_directory: Path, pages: list[dict[str, object]]) -> list[Path]:
    ...
    for index, page_payload in enumerate(pages, start=1):
        page_path: Path = pages_directory / f'page_{index:04d}.json'
        write_json(page_path, page_payload)
        saved_paths.append(page_path)
    result: list[Path] = saved_paths
    return result
...
def build_summary_markdown(collection_id: int, summary: dict[str, object]) -> str:
    ...
    Called by: main()
    """
    duplicate_filenames: object = summary['duplicate_filenames']
    anomaly_examples: object = summary['filename_anomaly_examples']
    lines: list[str] = [
...
def parse_args() -> argparse.Namespace:
    ...
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
        description='Capture Archive-It WASAPI metadata for one collection.',
    )
...
def main() -> None:
    ...
    wasapi_base_url: str = os.getenv('ARCHIVEIT_WASAPI_BASE_URL', DEFAULT_WASAPI_BASE_URL)

    requested_at: datetime = datetime.now(UTC)
    output_paths: dict[str, Path] = build_output_paths(Path(args.output_dir), args.collection_id, requested_at)
    ...
    fetch_result: FetchResult = FetchResult(pages=[], request_records=[])
    ...
    summary: dict[str, object] = build_metadata_summary(fetch_result.pages)
    manifest: dict[str, object] = build_capture_manifest(
        collection_id=args.collection_id,
        base_url=wasapi_base_url,
