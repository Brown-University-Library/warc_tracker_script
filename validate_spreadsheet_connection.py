...
def resolve_spreadsheet_id(cli_spreadsheet_id: str | None) -> str:
    ...
    if cli_spreadsheet_id is not None and cli_spreadsheet_id.strip():
        ...
        env_spreadsheet_id: str | None = os.getenv('GSHEET_SPREADSHEET_ID')
...
def run_validation(spreadsheet_id: str) -> int:
    ...
    try:
        sheet_context: CollectionSheetContext = validate_collection_sheet_connection(spreadsheet_id)
    except CollectionSheetContractError as exc:
        error_message: str = format_contract_error_message(exc)
        ...
        success_message: str = format_success_message(sheet_context)
...
def main() -> None:
    ...
    try:
        spreadsheet_id: str = resolve_spreadsheet_id(args.spreadsheet_id)
    except ValueError as exc:
        log.exception('Unable to resolve spreadsheet id.')
        print(str(exc), file=sys.stderr)
        raise SystemExit(1) from exc
    exit_code: int = run_validation(spreadsheet_id)
